_G.envim_mcp_tunnel_start = function()
  ...
  server:listen(128, vim.schedule_wrap(function(error)
    ...
    envim_connect(0, { "envim_mcp_open", connection_id })

    client:read_start(vim.schedule_wrap(function(_, data)
      if data then
        envim_connect(0, { "envim_mcp_data", connection_id, vim.base64.encode(data) })
      else
        tunnel.clients[connection_id] = nil
        client:close()
        envim_connect(0, { "envim_mcp_close", connection_id })
      end
    end))
  end))
